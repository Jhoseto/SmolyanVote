/**
 * Create Referendum - Interactive JavaScript
 * Handles all form interactions, validation, and dynamic content
 */
...
function initializeElements() {
    elements.form = document.querySelector('.referendum-form');
    ...
    elements.submitBtn = document.getElementById('submitReferendumBtn');
...
function initializeCharacterCounters() {
    ...
    setupCharacterCounter('topic', 150);
        ...
        <div class="option-group" data-option="${number}">
            ...
            <div class="option-input-group">
                <input type="text" class="form-control option-input" name="options" required maxlength="100" 
                       placeholder="Напр. Кандидат ${String.fromCharCode(64 + number)}, Проект ${['X', 'Y', 'Z', 'W', 'V', 'U', 'T', 'S', 'R', 'Q'][number-1]}, Идея ${number}">
...
function initializeFormValidation() {
    ...
    elements.topicInput.addEventListener('blur', validateTopic);
...
function validateTopic() {
    const value = elements.topicInput.value.trim();
    const isValid = value.length >= 10 && value.length <= 150;

    if (!isValid) {
        elements.topicInput.classList.add('error');
    } else {
        elements.topicInput.classList.remove('error');
    }
...
function updateModalSummary() {
    // Update topic
    document.getElementById('summaryTopic').textContent = elements.topicInput.value || '-';
    ...
    fetch(elements.form.action, {
        ...
        .then(response => {
            if (response.ok) {
                hideConfirmationModal();
                showSuccessModal();
                showAlert('Референдумът е създаден успешно!', 'success');
            } else {
        ...
        .catch(error => {
            ...
            showAlert('Възникна грешка при създаването на референдума. Моля, опитайте отново.', 'error');
        })
        .finally(() => {
...
console.log('✅ Create Referendum JavaScript initialized successfully');