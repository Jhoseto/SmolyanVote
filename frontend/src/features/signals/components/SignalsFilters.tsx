...
export function useActiveSignalsFilterCount(): number {
  ...
  return (
    ...
    (filters.boostedOnly ? 1 : 0) +
    (filters.highPriorityOnly ? 1 : 0) +
    (filters.resolvedOnly ? 1 : 0) +
    (filters.nearMe ? 1 : 0)
  );
}

export interface SignalsFiltersContentProps {
  stacked?: boolean;
  totalCount?: number;
...
export function SignalsFiltersContent({
  stacked = false,
  totalCount,
  filteredCount,
  isAdmin,
  ...
  return (
    <div className={cn("flex flex-col gap-3", stacked && "gap-4")}>
      ...
      {showCategoryChips && dataset ? <SignalsCategoryChips dataset={dataset} /> : null}

      <div className={cn("flex flex-wrap gap-2", stacked && "flex-col")}>
        <select
          value={filters.category ?? ""}
          onChange={(e) => setFilters({ category: (e.target.value || null) as never })}
          className={cn(selectClass, stacked ? "w-full" : "min-w-[140px] flex-1")}
        >
        ...
        <select
          ...
          className={cn(selectClass, stacked && "w-full")}
        >
      ...
      <div className={cn("flex flex-wrap gap-2", stacked && "flex-col items-stretch")}>
        <FilterChip stacked={stacked} active={filters.showInactive} onClick={() => setFilters({ showInactive: !filters.showInactive })}>
          <i className="bi bi-eye-slash" />
          Неактивни
        </FilterChip>
        <FilterChip stacked={stacked} active={filters.mineOnly} onClick={handleMineOnly} disabled={!user && !filters.mineOnly}>
          <i className="bi bi-person" />
          Моите
        </FilterChip>
        <FilterChip stacked={stacked} active={filters.boostedOnly} onClick={() => setFilters({ boostedOnly: !filters.boostedOnly })}>
          <i className="bi bi-arrow-up-circle" />
          Вдигнати от мен
        </FilterChip>
        <FilterChip stacked={stacked} active={filters.highPriorityOnly} onClick={() => setFilters({ highPriorityOnly: !filters.highPriorityOnly })}>
        ...
        <FilterChip stacked={stacked} active={filters.nearMe} onClick={() => setFilters({ nearMe: !filters.nearMe })}>
          <i className="bi bi-geo" />
          Близо до мен
        </FilterChip>
        {isAdmin ? (
          <FilterChip stacked={stacked} active={!!adminQuickMode} onClick={() => onAdminQuickModeChange?.(!adminQuickMode)}>
