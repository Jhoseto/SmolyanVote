...
/**
 * Contact form modal body — dynamically imported from `AppProviders` so the
 * form/validation bundle only loads once the modal is actually opened. The
 * `?contact=1` deep-link watcher lives separately in `ContactModalQuerySync`
 * (always mounted, negligible weight) so it can flip `isOpen` before this
 * chunk has even loaded.
 */
